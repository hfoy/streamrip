...
class RipCore(list):
    ...
    def parse_urls(self, url: str) -> List[Tuple[str, str, str]]:
        ...
        parsed.extend(
            ("qobuz", url_type, item_id)
            for url_type, item_id in URL_REGEX.findall(url)
        )
    ...
    def prompt_creds(self, source: str):
        ...
        :type source: str
        """
        use_auth_token = self.config.file["qobuz"].get("use_auth_token", False)
        if use_auth_token:
            secho("Enter Qobuz user ID:", fg="green")
            self.config.file["qobuz"]["email_or_userid"] = input()
            secho("Enter Qobuz auth token:", fg="green")
            self.config.file["qobuz"]["password_or_token"] = input()
        else:
            ...
            self.config.file["qobuz"]["password_or_token"] = md5(
